                ...
                if (section != "")
            {
                ...
                if (s.Contains("[name]") || s.Contains("[launchNumber]") || s.Contains("blocNumber]"))
                {
                    ...
                    if (vesselNumber > 1 || settings.addAlways)
                        s = s.Replace("[launchNumber]", (b.vesselRoman ? ToRoman(vesselNumber) : vesselNumber.ToString("X" + (1 +settings.launchNumberMinDigits).ToString())));

                    if (blocNumber > 1 && b.showBloc)
                    {
                        s = s.Replace("[blocNumber]", (b.blocRoman ? ToRoman(blocNumber) : blocNumber.ToString("X" + (1 +settings.blocNumberMinDigits).ToString())));
                    }
                    if (s != section)
